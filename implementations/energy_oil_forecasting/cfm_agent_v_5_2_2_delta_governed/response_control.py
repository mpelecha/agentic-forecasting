"""Structured-assessment retry and neutral-fallback control for CFM Agent
v5.2.2 Delta-Governed.

Forked from
:class:`energy_oil_forecasting.cfm_agent_v_5_2.response_control.StructuredAssessmentController`
because it hardcodes the base ``CfmContextAssessmentOutput``/``HorizonAction``
classes directly inside ``_parse``, ``_corrected_response``, and
``_neutral`` — rather than deriving them from whatever ``output_schema`` an
``AgentPredictor`` was configured with. Reusing it unchanged means every
parse, correction-retry, and neutral-fallback path silently reverts to the
old string-category schema (``center_action="no_change"``) regardless of
what schema this predictor actually uses — exactly the ``KeyError:
'no_change'`` this fork exists to fix, since the delta-governed policy
expects an integer rank, never that string.
"""
...
class AssessmentResolutionDeltaGoverned:
    assessment: CfmContextAssessmentOutputDeltaGoverned
    predictions: list[Prediction]
    audit: dict[str, Any]


class StructuredAssessmentControllerDeltaGoverned:
    ...
    @staticmethod
    def _neutral(packet_id: str, horizons: list[int]) -> CfmContextAssessmentOutputDeltaGoverned:
        return CfmContextAssessmentOutputDeltaGoverned(
        ...
        *,
        ...
        correction = {
            "instruction": (
                ...
                "Remember: center_action must be an INTEGER in {-2,-1,0,1,2}, never a string category "
                "like 'no_change' or 'small_up'."
            ),
            "required_research_packet_id": packet_id,
            "required_horizons_in_order": horizons,
            "validation_errors": errors,
            "prior_response": raw_response,
            "output_schema_template": CfmContextAssessmentOutputDeltaGoverned.prompt_schema_json(),
        }
    ...
    def _parse(
        ...
        *,
        ...
        try:
            assessment = CfmContextAssessmentOutputDeltaGoverned.model_validate_json(cleaned)
        except ValidationError:
            assessment = CfmContextAssessmentOutputDeltaGoverned.model_validate(json.loads(cleaned))
    ...
    def resolve(
        ...
        *,
    ...
    ) -> AssessmentResolutionDeltaGoverned:
        ...
        audit: dict[str, Any] = {
            "control_id": "structured_assessment_retry_v522_delta_governed",
        ...
        return AssessmentResolutionDeltaGoverned(assessment=assessment, predictions=predictions, audit=audit)


__all__ = ["AssessmentResolutionDeltaGoverned", "StructuredAssessmentControllerDeltaGoverned"]
