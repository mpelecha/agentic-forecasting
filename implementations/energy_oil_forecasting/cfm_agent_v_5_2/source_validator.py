"""Optional audit-only destination-page inspection for CFM Agent v5.2."""
...
from energy_oil_forecasting.cfm_agent_v_5_2.tools.url_safety import (
    SafeRedirectHandler,
    validate_public_url,
)
...
def _extract_passages(source_id: str, text: str, settings: CfmV52Settings) -> list[AuditPassage]:
...
def _inspect_source(source: ActiveSource, cutoff_date: str, settings: CfmV52Settings) -> AuditSourceRecord:
    warnings: list[str] = []
    ...
    try:
        ...
        request = Request(
            source.resolved_url,
            headers={
                "User-Agent": "CFM-Agent-v5.1-Audit/1.0",
...
def run_source_audit(sources: list[ActiveSource], cutoff_date: str, settings: CfmV52Settings) -> SourceAuditBundle:
