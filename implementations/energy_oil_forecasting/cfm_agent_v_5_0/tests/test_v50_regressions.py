"""Regression coverage for the v4.4 live failure and v5.0 controls."""
...
def test_public_suite_path_serializes_frequency(monkeypatch):
    tool = AuthoritativeSuiteTool(FakeService(), settings=CfmV50Settings(), covariate_series_ids=[])
...
def test_march_2_claim_source_subset_regression(packet):
    ...
    decision = EvidencePolicy(CfmV50Settings()).apply(assessment, packet, 5)
