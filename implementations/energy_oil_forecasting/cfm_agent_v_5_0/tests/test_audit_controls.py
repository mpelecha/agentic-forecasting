...
from energy_oil_forecasting.cfm_agent_v_5_0.schemas import EvidenceClaim, HorizonAction
...
def test_source_audit_off_does_not_fetch(packet):
    bundle = run_source_audit(packet.sources, packet.cutoff_date, CfmV50Settings())
...
#     )
#     findings, audit = audit_claim_support(assessment, packet, CfmV50Settings())
...
def test_claim_audit_fails_closed_when_semantic_verifier_fails(
    ...
    findings, audit = audit_claim_support(
        assessment,
        packet,
        CfmV50Settings(audit_enabled=True),
    )
