...
from energy_oil_forecasting.cfm_agent_v_5_0.tools.code_execution import (
    AuditedCodeExecutionTool,
)
from energy_oil_forecasting.cfm_agent_v_5_0.tools.market_data import (
...
def test_real_model_suite_construction_includes_frequency_and_round_trips():
    tool = AuthoritativeSuiteTool(SyntheticService(), settings=CfmV50Settings(), covariate_series_ids=[])
    _stub_models(tool)
...
def test_public_authoritative_tool_constructs_and_serializes_suite():
    ...
    tool = AuthoritativeSuiteTool(service, settings=CfmV50Settings(), covariate_series_ids=[])
    _stub_models(tool)
    task = _task()
...
def test_code_execution_audit_defaults_to_unused():
    audit = AuditedCodeExecutionTool(CfmV50Settings()).audit
    assert audit == {
...
def test_market_tool_signature_hides_context_from_llm_schema():
    ...
    schema = (
        AuthoritativeSuiteTool(SyntheticService(), settings=CfmV50Settings(), covariate_series_ids=[])
        .as_function_tool()
        ._get_declaration()
...
def test_neutral_engine_is_literal_reproduction():
    ...
    result = PythonForecastEngine(CfmV50Settings()).transform(
