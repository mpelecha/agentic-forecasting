"""End-to-end v5.0 controller with task binding and isolated audits."""
...
from energy_oil_forecasting.cfm_agent_v_5_0.claim_support_verifier import (
    audit_claim_support,
)
...
from energy_oil_forecasting.cfm_agent_v_5_0.prompts import CfmV50PromptBuilder
...
class CfmV50Predictor(Predictor):
    def __init__(
        self,
        config: AgentConfig,
        ...
        self.inner = AgentPredictor(
            agent_config=config,
            prompt_builder=CfmV50PromptBuilder(),
    ...
    def predict(  # noqa: PLR0912, PLR0915
        ...
        placeholders = self.inner.predict(task, context)
        ...
        assessment = CfmContextAssessmentOutput.model_validate(placeholders[0].metadata.get("context_assessment"))
        if assessment.research_packet_id != packet.packet_id:
            raise RuntimeError("LLM response references the wrong research packet")
        if [action.horizon for action in assessment.horizon_actions] != expected_horizons:
            raise RuntimeError("LLM horizon actions must exactly match task horizons in task order")
        if self.settings.audit_enabled:
            findings, support_audit = audit_claim_support(assessment, packet, self.settings)
        else:
            findings = []
            support_audit = {
                "verifier_id": "claim_support_verifier_v50_audit_only",
                "audit_only": True,
                "executed": False,
                "completed": False,
